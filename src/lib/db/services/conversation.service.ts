...
import type { IConversation, IUser, CreateConversationData } from '@/types/database';
 ...
 */
function createErrorResult(code: string, message: string, details?: any): ConversationServiceResult {
  return {
    success: false,
...
export async function createConversation(
  ...
  try {
    ...
    // Verify user exists
    const user = await User.findOne({ clerkId: input.clerkId }).session(session || null);
    if (!user) {
    ...
    console.log(`[ConversationService.createConversation] Created conversation: ${conversation._id} for user: ${input.clerkId}`);
    ...
    if (error.name === 'ValidationError') {
      return createErrorResult('VALIDATION_ERROR', 'Conversation data validation failed', error.errors);
    }
    
    return createErrorResult('DATABASE_ERROR', 'Failed to create conversation', error.message);
  }
}
...
export async function getConversationById(
  ...
  try {
    ...
    const conversation = await Conversation.findOne(query)
      .populate('messages', 'content role createdAt', undefined, { sort: { createdAt: 1 } });
    ...
    return createErrorResult('DATABASE_ERROR', 'Failed to fetch conversation', error.message);
  }
}
...
export async function updateConversation(
  ...
  try {
    ...
    if (updates.aiModel !== undefined) settingsUpdates['settings.aiModel'] = updates.aiModel;
    if (updates.temperature !== undefined) settingsUpdates['settings.temperature'] = updates.temperature;
    if (updates.maxTokens !== undefined) settingsUpdates['settings.maxTokens'] = updates.maxTokens;
    if (updates.systemMessage !== undefined) settingsUpdates['settings.systemMessage'] = updates.systemMessage;
    if (updates.isPublic !== undefined) settingsUpdates['settings.isPublic'] = updates.isPublic;
    ...
    console.log(`[ConversationService.updateConversation] Updated conversation: ${conversationId}`);
    ...
    if (error.name === 'ValidationError') {
      return createErrorResult('VALIDATION_ERROR', 'Update data validation failed', error.errors);
    }
    
    return createErrorResult('DATABASE_ERROR', 'Failed to update conversation', error.message);
  }
}
...
export async function deleteConversation(
  ...
  try {
    ...
    if (hardDelete) {
      ...
      console.log(`[ConversationService.deleteConversation] Hard deleted conversation: ${conversationId}`);
    } else {
      ...
      console.log(`[ConversationService.deleteConversation] Soft deleted conversation: ${conversationId}`);
    }

    return createSuccessResult({ deleted: true });

  } catch (error: any) {
    logError('deleteConversation', error, { conversationId, clerkId, hardDelete });
    return createErrorResult('DATABASE_ERROR', 'Failed to delete conversation', error.message);
  }
}
...
export async function archiveConversation(
  ...
  try {
    ...
    const archivedConversation = await Conversation.archiveConversation(conversationId);
    
    if (!archivedConversation) {
      return createErrorResult('ARCHIVE_FAILED', 'Failed to archive conversation');
    }

    console.log(`[ConversationService.archiveConversation] Archived conversation: ${conversationId}`);
    
    return createSuccessResult(archivedConversation);

  } catch (error: any) {
    logError('archiveConversation', error, { conversationId, clerkId });
    return createErrorResult('DATABASE_ERROR', 'Failed to archive conversation', error.message);
  }
}
...
export async function toggleConversationPin(
  ...
  try {
    ...
    if (!updatedConversation) {
      return createErrorResult('PIN_FAILED', `Failed to ${pin ? 'pin' : 'unpin'} conversation`);
    }

    console.log(`[ConversationService.toggleConversationPin] ${pin ? 'Pinned' : 'Unpinned'} conversation: ${conversationId}`);
    
    return createSuccessResult(updatedConversation);

  } catch (error: any) {
    logError('toggleConversationPin', error, { conversationId, clerkId, pin });
    return createErrorResult('DATABASE_ERROR', `Failed to ${pin ? 'pin' : 'unpin'} conversation`, error.message);
  }
}
...
export async function getUserConversations(
  ...
  try {
    ...
    const conversations = await Conversation.find(query)
      ...
      .skip(skip)
      .limit(limit)
      .populate('messages', 'content role createdAt', undefined, { sort: { createdAt: -1 }, limit: 1 });

    return createSuccessResult(conversations);

  } catch (error: any) {
    logError('getUserConversations', error, { clerkId, options });
    return createErrorResult('DATABASE_ERROR', 'Failed to fetch user conversations', error.message);
  }
}
...
export async function searchUserConversations(
  ...
  try {
    ...
    const conversations = await Conversation.searchUserConversations(clerkId, query.trim(), limit);
    
    return createSuccessResult(conversations);

  } catch (error: any) {
    logError('searchUserConversations', error, { clerkId, query });
    return createErrorResult('DATABASE_ERROR', 'Failed to search conversations', error.message);
  }
}
